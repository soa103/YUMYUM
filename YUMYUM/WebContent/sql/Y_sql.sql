...
-- 닉네임 변경 시 다른 테이블의 닉네임도 같이 변경하는 트리거 --
CREATE OR REPLACE TRIGGER NICKNAME_TRIGGER
   AFTER UPDATE OF USER_NICKNAME ON USER_TBL FOR EACH ROW
   BEGIN 
      UPDATE REVIEW_TBL
       SET RE_WRITER = :NEW.USER_NICKNAME
       WHERE RE_WRITER = :OLD.USER_NICKNAME;
      UPDATE REPLY_TBL 
      SET REPLY_WRITER=:NEW.USER_NICKNAME
      WHERE REPLY_WRITER=:OLD.USER_NICKNAME;
      UPDATE MP_APPROVAL_NEED_TBL 
      SET MPA_WRITER =:NEW.USER_NICKNAME 
      WHERE MPA_WRITER =:OLD.USER_NICKNAME;
      UPDATE BOARD_TBL 
      SET BOARD_WRITER=:NEW.USER_NICKNAME
      WHERE BOARD_WRITER=:OLD.USER_NICKNAME;
   END;
/
